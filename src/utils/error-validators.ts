...
import { ApiFormValidationError, ErrorDetail } from "@/types/error-detail";
...
export const isApiFormValidationErrorResponse = (
  ...
  Object.keys(error.response?.data?.errors).every(
    (key) =>
      ...
      error.response?.data.errors[key].every(
        (err: string) => typeof err === "string",
      ),
  );
...
export const isErrorDetailResponse = (
  ...
  typeof error.respAnyUseInfiniteQueryOptionse?.data?.title === "string" &&
